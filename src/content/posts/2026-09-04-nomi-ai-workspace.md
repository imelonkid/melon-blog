...
工具不是一个个硬编码进去的，而是统一描述。每份描述除了"这个工具是什么、怎么执行"，还要说清楚四件事：
...
风险等级写进描述里还有个额外的好处：**能力开放变成了可以配置的事**。哪些工具对哪些人可用、哪些必须走审批，是企业自己定的，不用交给模型临场判断。
...
还有个细节：确认状态是持久化的。用户点了同意，这个决定会跟着任务走到后续请求里继续执行，刷新页面或者换个会话也不会丢，而不是每一轮重新问一遍。
...
也是在这个过程里我改了对"好用"的理解。一开始我以为目标是让人更会写 Prompt，后来发现恰好相反——**用户需要解释的东西越少，这个系统才算做对了**。
